...
export class Tab2Page implements AfterViewInit {
  ...
  /**
   * Initializes the profile image after the view is initialized.
   */
  ...
  /**
   * Determines which profile display type should be used.
   */
  private setProfileImage(): void {
    if(this.profileImageUri){
      this.applyDeviceProfileImage(this.profileImageUri.toString());
    }else{
      this.setUserInitialsImage();
    }
  }

  /**
   * Forms the initials to display when the profileDisplayType is INITIALS.
   */
  private setUserInitialsImage(): void {
    this.profileDisplayType = ProfileDisplayType.INITIALS;
  }
        ...
        this.cropProfileImage(imagePath)
          .then((result: ProfileImageCropResult) => {
            ...
            const onError: (error: Error) => void = (error: Error): void => {
              this.presentToast('onError :: Messages.profileImageStoreError');
            };
