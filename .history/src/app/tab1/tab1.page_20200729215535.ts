...
export class Tab1Page implements AfterViewInit{
    ...
    this.fileTransfer
      ...
      .then(
        (entry) => {
          ...
          return this.checkFileExists(this.file.cacheDirectory, 'Privacy-Statement_tcm145-107975.pdf');
        },
        (error) => {
          // handle error
          this.presentToast(JSON.stringify(error));
  ...
  askStoragePermissions(): Promise<any>{
    return this.diagnostic.requestRuntimePermissions([this.diagnostic.permission.READ_EXTERNAL_STORAGE, this.diagnostic.permission.WRITE_EXTERNAL_STORAGE]).then((status: any) =>{
      this.logs.push(`status :: ${JSON.stringify(status)} :: ${this.diagnostic.permissionStatus.GRANTED}`);
      if (status.WRITE_EXTERNAL_STORAGE === this.diagnostic.permissionStatus.GRANTED){
        return this.downloadToRootDir();
      }else{
        this.logs.push(`No Download :: ${JSON.stringify(status)} :: ${this.diagnostic.permissionStatus.GRANTED}`);
