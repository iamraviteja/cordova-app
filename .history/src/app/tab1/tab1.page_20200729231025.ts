...
export class Tab1Page implements AfterViewInit {
  ...
  checkFileNo(name: string) : string{
    let len = this.fileList.filter(item => item.startsWith(name)).length;
    if(len === 0){
      return name;
    }else{
      return `${name}_${len}`;
  ...
  onCheckCopyFileData(): void {
    this.fileTransfer
      .download(this.url, this.getDownloadPath(this.file.cacheDirectory))
      .then(
        (entry) => {
      ...
      .then(
        (entry) => {
          ...
          if (!entry) {
            ...
            return Promise.reject("This is a duplicate file");
          }
        },
        (error) => {
          // handle error
